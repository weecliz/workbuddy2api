...
rem 2a. project venv (relative to this script, so it works from anywhere)
...
rem 2b. the WorkBuddy desktop app's bundled interpreter
if not defined PY for %%P in (
...
rem  Probe a candidate interpreter and set PY only if it has the packages this
rem  script needs. Interpreter discovery must not blindly trust PATH: the first
rem  `python` on a machine is often a bare CPython without fastapi/sqlalchemy,
rem  and picking it produced a confusing "missing dependencies" abort even
rem  though the project venv next to this script was fully populated.
rem  %%W / %%P from the caller's for-loop are not visible in here, hence the
rem  "call :try_py" indirection.
rem ------------------------------------------------------------
:try_py
"%~1" -c "import fastapi, uvicorn, sqlalchemy" >nul 2>&1
